...
[ApiController]
public class PutPlanSetActiveController : CustomControllerBaseExtension, IOutputPort<Domain.Entities.Plan>
{
    private IActionResult _viewModel;
    private readonly NotificationHelper _notificationHelper;

    public PutPlanSetActiveController(
    ...
    [SwaggerOperation(Summary = "Modificar campo ativo de um plano por id")]
    public async Task<IActionResult> PutPlanSetActive(
        [FromServices] IPutPlanSetActiveUseCase useCase,
        [FromRoute][Required] long id,
        [FromBody][Required] PutPlanSetActiveRequest putPlanSetActiveRequest)
    {
        useCase.SetOutputPort(this);

        await useCase.Execute(id, putPlanSetActiveRequest.Ativo);
